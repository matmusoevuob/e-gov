...
import { LoginSchema, VerifyCertSchema, SsoSchema } from '../types/index.js';
import { PkiService } from '../services/pki.service.js';
import { KeycloakService } from '../services/keycloak.service.js';
import { DbService } from '../services/db.service.js';
import jwt from 'jsonwebtoken';

export function createAuthRouter(pkiService: PkiService, keycloakService: KeycloakService, dbService: DbService): Router {
  const router = Router();
   ...
   */
  router.post('/verify-cert', (req: Request, res: Response) => {
    try {
      const parseResult = VerifyCertSchema.safeParse(req.body);
      if (!parseResult.success) {
        return res.status(400).json({
          status: 'ERROR',
          code: 'INVALID_INPUT',
  ...
  router.post('/sso', async (req: Request, res: Response) => {
    try {
      const parseResult = SsoSchema.safeParse(req.body);
      if (!parseResult.success) {
        return res.status(400).json({
          status: 'ERROR',
          code: 'INVALID_INPUT',
